...
interwar <- ggraph(netw1, layout = "fr", maxiter=100000000)+
  ...
  labs(title="Suomalaisten puolueiden hallituskumppanit rauhan aikana 1918–1939",
...
postwar <- ggraph(netw2, layout = "fr", maxiter=10000000)+
  geom_edge_link(aes(width=weight, alpha=weight, start_cap = label_rect(node1.name),
                     end_cap = label_rect(node2.name)))+
  geom_node_label(aes(label=name, fill=name), colour="white", size=6, label.padding = unit(0.45, "lines"))+
  geom_shadowtext(aes(x, y, label=name), size=6)+
  ...
  scale_fill_manual(values = colours)+
  labs(title="Suomalaisten puolueiden hallituskumppanit rauhan aikana 1945–1991",
       subtitle = "Mitä paksumpi ja näkyvämpi viiva puolueiden välillä,\nsitä useamman päivän samassa hallituksessa",
       caption = "Julius Lehtinen / Databyro"
  )+
  coord_cartesian(clip="off")+
...
coop <- ggraph(netw3, layout = "fr", maxiter=10000000)+
  geom_edge_link(aes(width=weight, alpha=weight, start_cap = label_rect(node1.name),
                     end_cap = label_rect(node2.name)))+
  geom_node_label(aes(label=name, fill=name), colour="white", size=6, label.padding = unit(0.45, "lines"))+
  geom_shadowtext(aes(x, y, label=name), size=6)+
  ...
  scale_fill_manual(values = colours)+
  labs(title="Suomalaisten puolueiden hallituskumppanit rauhan aikana 1991–2022",
       subtitle = "Mitä paksumpi ja näkyvämpi viiva puolueiden välillä,\nsitä useamman päivän samassa hallituksessa",
       caption = "Julius Lehtinen / Databyro"
  )+
  coord_cartesian(clip="off")+
...
total <- ggraph(netw4, layout = "fr", maxiter=100000000)+
  geom_edge_link(aes(width=weight, alpha=weight, start_cap = label_rect(node1.name),
                     end_cap = label_rect(node2.name)))+
  geom_node_label(aes(label=name, fill=name), colour="white", size=6, label.padding = unit(0.45, "lines"))+
  geom_shadowtext(aes(x, y, label=name), size=6)+
  ...
  scale_fill_manual(values = colours)+
  labs(title="Suomalaisten puolueiden hallituskumppanit rauhan aikana 1918–2022",
       subtitle = "Mitä paksumpi ja näkyvämpi viiva puolueiden välillä,\nsitä useamman päivän samassa hallituksessa",
       caption = "Julius Lehtinen / Databyro"
       )+
  coord_cartesian(clip="off")+
...
ggsave("network_interwar.png", dpi = 1000, plot = interwar, height = 225, width = 400, units = "mm")
ggsave("network_postwar.png", dpi = 1000, plot = postwar, height = 225, width = 400, units = "mm")
ggsave("network_coop.png", dpi = 1000, plot = coop, height = 225, width = 400, units = "mm")
ggsave("network_total.png", dpi = 1000, plot = total, height = 225, width = 400, units = "mm")
